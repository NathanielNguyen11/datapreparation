...
IM = '/media/ubuntu/Quang/challenge/liver/CT/experi/train/1'
LB = '/media/ubuntu/Quang/challenge/liver/CT/experi/train/1_LB'
...
def circle(lb,im,name):
	...
	for j in range(0,len(cnts)): 
		...
		print cv2.contourArea(cnts[j]) 
		if cv2.contourArea(cnts[j]) < 500:
			print cnts[j]
			# kernel = np.ones((10,10),np.uint8)
			# erosion = cv2.erode(cnts[j],kernel,iterations = 1)
		else:
			print cnts[j]
				# print area
